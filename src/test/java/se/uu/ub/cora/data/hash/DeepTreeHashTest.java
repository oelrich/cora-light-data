...
public class DeepTreeHashTest {
	...
	void loadJsonAndGetUniqueNamesAndGetUniqueValues() {
		try (var file = getClass().getResourceAsStream(LINK_LIST_CORA);
			...
			linkListsCoraJson = new JSONObject(reader.lines()
	...
	void testRobustness() {
		...
		assertTrue(names.stream()
			.allMatch(name -> name.equals(dth.string(dth.hash(name)))));
		assertTrue(values.stream()
			.allMatch(value -> value.equals(dth.string(dth.hash(value)))));
	}
